locals {
  consul_server_task_container_definitions = templatefile("${path.root}/container-definitions/consul-server.json.tpl", {
    environment_object_path = "s3://${var.secrets_bucket_name}/${local.consul_server_environment_object_key}"
...
module "consul_server_ecs_service" {
  ...
  service_task_container_definitions = local.consul_server_task_container_definitions
